...
# Your code here
df_typed["total_spent"] = df_typed["total_spent"].astype(str).str.replace("$", "", regex=False)
...
df_typed["age"] = df_typed["age"].fillna(df_typed["age"].median()).astype(int)
...
# Your code here
df_no_duplicates["days_since_last_purchase"] = (pd.Timestamp.today() - df_no_duplicates["last_purchase"]).dt.days
...
df_no_duplicates["average_purchase_value"] = df_no_duplicates["average_purchase_value"].replace([np.inf, -np.inf], 0)
df_no_duplicates["average_purchase_value"] = df_no_duplicates["average_purchase_value"].fillna(0)
...
# Your code here
df_final = df_renamed.drop(columns=[])
...
print("- Data Type Issues: Mixed date formats, currency symbols/commas in total_spent, inconsistent numeric/text formatting")
